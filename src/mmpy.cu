...
void verify(_FTYPE_ *c, _FTYPE_ *a, _FTYPE_ *b, unsigned int m, unsigned int n,
            _FTYPE_ epsilon, const char *mesg);
void verify_bt(_FTYPE_ *c, unsigned int n, const char *mesg);
void verify(_FTYPE_ *c_d, _FTYPE_ *c_h, unsigned int m, unsigned int n,
            _FTYPE_ eps, const char *mesg);
void verify_bt(_FTYPE_ *c_d, _FTYPE_ *c_h, unsigned int n, const char *mesg);
void verify_bt(_FTYPE_ *c_d, _FTYPE_ *c_h, unsigned int m, unsigned int n,
               const char *mesg);
void verify_ISeq(_FTYPE_ *c, unsigned int m, unsigned int n);
void verify_rand(_FTYPE_ *a, _FTYPE_ *b, _FTYPE_ *c, unsigned int n,
                 _FTYPE_ eps);

void printMatrix(_FTYPE_ *a, unsigned int m, unsigned int n);
void cmdLine(int argc, char *argv[], int &n, int &reps, int &ntx, int &nty,
             _FTYPE_ &eps, int &do_host, int &prefer_l1, int &use_rand,
             int &use_bt, int &use_seq, int &use_shm_double, int &verify_gpu);
void perfString(int n, int ntx, int nty, int reps, double t_h, double gflops_h,
                double t_d, double gflops_d, int do_host, int prefer_l1,
                int use_rand, int use_bt, int use_shm_double);

double getTime();
double gflops(int n, int niter, double time);

void matMulHost(_FTYPE_ *, const _FTYPE_ *, const _FTYPE_ *, unsigned int,
                unsigned int);
...
int main(int argc, char **argv) {
  ...
  int n, reps, ntx, nty, do_host, prefer_l1, use_rand, use_bt, use_seq,
      use_shm_double, verify_gpu;
  _FTYPE_ eps;

  cmdLine(argc, argv, n, reps, ntx, nty, eps, do_host, prefer_l1, use_rand,
          use_bt, use_seq, use_shm_double, verify_gpu);
  ...
  printf(
      "n: %d, tx: %d, ty: %d, gridX: %d, gridY: %d, reps: %d, epsilon: %g\n\n",
  ...
  // shared memory, we may run slightly faster on devices that have the
  // capability
...
// execute the kernel
  ...
  for (int r = 0; r < SCALE * reps; r++)
    cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n, &cublas_alpha,
                d_B, n, d_A, n, &cublas_beta, d_C, n);
#else
  for (int r = 0; r < SCALE * reps; r++)
    cublasDgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n, &cublas_alpha,
                d_B, n, d_A, n, &cublas_beta, d_C, n);
#endif
  ...
  perfString(n, ntx, nty, reps, t_host, gflops_h, t_device, gflops_d, do_host,
             prefer_l1, use_rand, use_bt, use_shm_double);
